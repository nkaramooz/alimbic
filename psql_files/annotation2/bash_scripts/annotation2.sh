#!/bin/bash
echo "here we go"
psql -d laso -c "drop table annotation2.cleaned_selected_descriptions_prelim;"
echo "running manual_description_filters.py"
python /Users/LilNimster/Documents/cascade/manual_description_filters.py

echo "cleaned_selected_descriptions_de_duped.sql"
psql -d laso -f ../cleaned_selected_descriptions_de_duped.sql -q
echo "snomed_synonyms.sql"
psql -d laso -f ../snomed_synonyms.sql -q
echo "snomed_cid_ignore.sql"
psql -d laso -f ../snomed_cid_ignore.sql -q
echo "acronym_augmented_descriptions.sql"
psql -d laso -f ../acronym_augmented_descriptions.sql -q

echo "insert_snomed_cid_to_root.sql"
psql -d laso -f ../insert_to_root/insert_snomed_cid_to_upstream_root.sql -q
echo "insert_new_concepts_to_root.sql"
psql -d laso -f ../insert_to_root/insert_new_concepts_to_upstream_root.sql -q

echo "insert_snomed_did_to_root.sql"
psql -d laso -f ../insert_to_root/insert_snomed_did_to_upstream_root.sql -q
echo "insert_therapies_syn_to_root.sql"
psql -d laso -f ../insert_to_root/insert_therapies_syn_to_upstream_root.sql -q
echo "insert_acronyms_to_root.sql"
psql -d laso -f ../insert_to_root/insert_acronyms_to_upstream_root.sql -q
echo "insert_root_new_desc_to_root.sql"
psql -d laso -f ../insert_to_root/insert_root_new_desc_to_root.sql -q
echo "insert_to_downstream.sql"
psql -d laso -f ../insert_to_root/insert_to_downstream_root.sql -q
echo "add_adid_acronym.sql"
psql -d laso -f ../manual_tables/add_adid_acronym.sql -q
echo "create_custom_terms.py"
python /Users/LilNimster/Documents/cascade/psql_files/annotation2/manual_tables/create_custom_terms.py
echo "rerun add_adid_acronym.sql now with custom_terms"
psql -d laso -f ../manual_tables/add_adid_acronym.sql -q
echo "lemmatizer.py"
python /Users/LilNimster/Documents/cascade/psql_files/annotation2/bash_scripts/lemmatizer.py
echo "first_word"
psql -d laso -f ../annotator_tables/first_word.sql